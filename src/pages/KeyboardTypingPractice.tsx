...
export function KeyboardTypingPractice() {
  const navigate = useNavigate();
  const { markComplete } = useUser();
  const savedKeyboardTracks = getStudyResult('typing')?.details?.keyboardTracks ?? {};
  ...
  const finishTrack = (finalTypingStats: TypingStats = typingStats) => {
    ...
    const nextTrackResults = { ...trackResults, [track]: result };
    setTrackResults(nextTrackResults);
    saveStudyResult('typing', moduleStartedAtRef.current, { keyboardTracks: nextTrackResults });
    if (nextTrackResults.ko && nextTrackResults.en && !completingRef.current) {
      completingRef.current = true;
      playFanfare();
      markComplete('typing');
      window.setTimeout(() => navigate('/result/typing'), 900);
    } else {
      playFanfare();
    }
    setStage('complete');
